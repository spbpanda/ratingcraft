 ...
 */
const createUnconnectedPingFrame = (timestamp: string | number | bigint | boolean) => {
 ...
 */
const extractModt = (unconnectedPongPacket: Buffer<ArrayBufferLike>) => {
	if (
 ...
 */
const ping = (host: string | undefined, port = 19132, cb: { (res: any, err: any): void; (arg0: { edition: string; name: string; version: { protocolVersion: number; minecraftVersion: string; }; players: { online: number; max: number; }; serverId: string; mapName: string; gameMode: string; } | null, arg1: null): void; }, timeout = 5000) => {
 ...
 */
export const pingBedrock = (host: any, options = {}) => {
	if (!host) throw new Error("Host argument is not provided");

	const { port = 19132, timeout = 5000 }: any = options;
