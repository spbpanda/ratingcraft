...
import { CountedCaption } from '../models/counted-caption.model';
...
export class SbiCountedCaptionPipe implements PipeTransform {
   ...
   * @param {CountedCaption} captions - Объект с тремя вариантами склонения
   * @returns {string} - Строка с числом и правильным склонением
   */
  transform(value: number | null, captions: CountedCaption): string {
    const count = value ?? 0;
