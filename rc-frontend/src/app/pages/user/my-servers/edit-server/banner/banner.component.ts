...
export class BannerComponent {
  @Input() maxSizeInBytes = 2000; // Максимальный размер баннера 
  @Input() bannerControl = new FormControl(''); // FormControl для баннера
  trashBin = TRASH_BIN;
  ...
  validateFileSize = (maxSize: number): ValidatorFn => {
    return (control: AbstractControl): ValidationErrors | null => {
      ...
      if (file && file.size > maxSize) {
        return { fileSize: `Размер файла не должен превышать ${maxSize / 1024} КБ.` };
      }
      return null;
    };
  }
