...
export class ImagesComponent {
  ...
  onFileSelected(event: Event) {
    ...
    if (input.files) {
      ...
      for (const file of files) {
        ...
        if (file.size > this.maxSizeInBytes * 1024) {
          alert(`Файл "${file.name}" превышает ${this.maxSizeInBytes} КБ и не будет загружен.`);
          continue;
        }
        ...
        reader.onload = () => {
          ...
          this.images.push(base64Image); // Добавляем изображение в массив
  ...
  removeImage(index: number) {
    ...
    this.imagesChange.emit([...this.images]); // Передаём обновления
  }
}